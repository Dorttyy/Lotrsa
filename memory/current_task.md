...
## Next step
Obtain original server config securely, or explicit recovery/reprovision authorization including session invalidation consequences. Do not create fake successful signup or seed over existing accounts. After approved recovery: backend testing first, then request user permission for UI testing, validate signup -> onboarding -> persistence/login. Implement reference-inspired onboarding as a separate reviewed increment.
